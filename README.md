Student Details
Name: Niharika Pathak
Registration Number: 25BAI11288
Branch: B.Tech (CSE with AI/ML)
Year: First Year

 Project Overview
This repository contains my Digital Literacy Project for the course CSE0001. The project focuses on improving digital awareness, building a professional online presence, understanding communication etiquette, and staying safe from cyber threats.

 Repository Structure
report/ → Final Project Report
task-1-presentation/ → Infographic design
task-2-portfolio/ → Profile screenshots
task-3-platforms/ → Coding & Google Form screenshots
task-4-email-etiquette/ → Email drafts & checklist
task-5-cybercrime/ → Case study & prevention checklist

 Task Summary
 
* Task 1: Digital Literacy Infographic
Created an awareness infographic using Canva covering digital literacy concepts and safe internet practices.

* Task 2: Digital Portfolio
Set up professional profiles on GitHub and LinkedIn.

* Task 3: Platforms
Completed a coding challenge on HackerRank and created a Google Form quiz.

 Google Form Link:https://docs.google.com/forms/d/e/1FAIpQLSfTbz0PIJ8Kjxnsx-48D9sbOYfCnjL8-Sat0N08hgbU8vUOqA/viewform?usp=publish-editor


 
* Task 4: Email Etiquette
Drafted professional emails and created a social media checklist.

* Task 5: Cybercrime Awareness
Prepared a case study on UPI fraud and created a prevention checklist.

Learning Outcomes
Improved digital communication skills
Learned professional email writing
Built an online portfolio
Gained awareness about cybersecurity
Explored useful digital tools

 References
https://canva.com
https://github.com
https://linkedin.com
https://hackerrank.com
https://forms.google.com
https://cybercrime.gov.in
